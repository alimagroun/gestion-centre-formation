...
public class PersonService {
    ...
    if (parentSaved == null) {
      if (isPhoneNumberUnique(parent.getPhoneNumber())) {
        throw new DuplicateEntityException(
            "Phone number " + parent.getPhoneNumber() + " already exists");
      }
  ...
  public PageResponse<ParentResponse> findAllParentsPaged(int page, int size) {
    ...
    List<ParentResponse> parentResponses =
        parentsPage.stream().map(parentMapper::toResponse).toList();
  ...
  public List<StudentResponse> findAllStudentsByParentId(Long parentId) {
    List<StudentResponse> studentList =
        personRepository.findAllStudentsByParentId(parentId).stream()
  ...
  public ParentDetailResponse findParentDetailById(Long parentId) {
    Parent parent =
        personRepository
            ...
            .orElseThrow(() -> new NotFoundException("Parent with ID " + parentId + " not found"));
  ...
  public PageResponse<StudentResponse> findAllStudentsPaged(int page, int size) {
    ...
    List<StudentResponse> studentList =
        studentsPage.stream().map(studentMapper::toResponse).toList();
    return new PageResponse<>(
        studentList,
  ...
  public PageResponse<TeacherResponse> getTeachersPage(int page, int size) {
    ...
    List<TeacherResponse> teacherResponses =
        teacherPage.stream().map(teacherMapper::toTeacherResponse).toList();
    return new PageResponse<>(
        teacherResponses,
