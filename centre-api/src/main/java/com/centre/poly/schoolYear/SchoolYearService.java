...
public class SchoolYearService {
  ...
  public Long save(SchoolYearRequest request) {
    ...
    SchoolYear verify =
        schoolYearRepository.findByStarEndYear(request.startYear(), request.endYear());
    if (verify != null) {
  ...
  public PageResponse<SchoolYearResponse> findAllPagebale(int page, int size) {
    ...
    List<SchoolYearResponse> schoolYearResponses =
        pageSchoolYear.map(schoolYearMapper::toResponse).toList();
    return new PageResponse<>(
        schoolYearResponses,
        pageSchoolYear.getNumber(),
        pageSchoolYear.getSize(),
        pageSchoolYear.getTotalElements(),
        pageSchoolYear.getTotalPages(),
        pageSchoolYear.isFirst(),
        pageSchoolYear.isLast());
  }
  ...
  // Only one school year can have isDefault = true. Before setting a new school year as the
  // default, the previous default year must be set to false
  public void toDefault(Long id) {

    SchoolYear schoolYearToDefault =
        schoolYearRepository
            .findById(id)
            .orElseThrow(() -> new NotFoundException("School year not found"));
  ...
  public SchoolYear getDefaultSchoolYear() {
    return schoolYearRepository
        .findFirstByIsDefaultTrue()
        .orElseThrow(() -> new NotFoundException("No default school year found"));
  }
}
