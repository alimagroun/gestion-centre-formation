...
import com.centre.poly.exportDocument.PdfService;
...
public class ClassService {
  ...
  public Long saveAccreditedClass(AccreditedClassRequest request) {
    ...
    SchoolYear schoolYear =
        schoolYearRepository
            .findById(request.getSchoolYearId())
            .orElseThrow(() -> new NotFoundException("School year not found"));

    classeFormationRepository
        .findAccreditedClass(
  ...
  public List<ClassStudentResponse> findAllStudentByAcceleratedClassId(Long classId) {
    List<AcceleratedClassEntry> entryList =
        acceleratedClassEntryRepository.findAllByClassId(classId);
    List<ClassStudentResponse> responses =
        entryList.stream().map(classMapper::toClassStudentResponse).toList();
    return responses;
  }
