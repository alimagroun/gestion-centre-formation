...
public class RegistrationController {
    ...
    public ResponseEntity<Long> register(@Valid @RequestBody RegistrationRequest request) {
        return ResponseEntity.ok()
                             .body(registrationService.save(request));
    }
    
    @PreAuthorize("hasRole('ROLE_ADMIN')")
    @GetMapping
    public ResponseEntity<PageResponse<RegistrationResponse>> findAllRegistrations(@RequestParam(name = "page",
                                                                                                 defaultValue = "0",
                                                                                                 required = false) int page, @RequestParam(name = "size",
                                                                                                                                           defaultValue = "10",
                                                                                                                                           required = false) int size) {
        return ResponseEntity.ok()
                             .body(registrationService.findAll(page, size));
    }
    ...
    public ResponseEntity<RegistrationDetailsResponse> findRegistrationById(@PathVariable Long id) {
        return ResponseEntity.ok()
                             .body(registrationService.findById(id));
    }
    ...
    public ResponseEntity<Long> updateRegistrationStatus(
        ...
        Long updatedRegistrationId = registrationService.updateRegistrationStatus(registrationId,
                                                                                  status,
                                                                                  statusChangeReason);
        return ResponseEntity.ok()
                             .body(updatedRegistrationId);
